...
def parse_arguments():
    ...
    parser.add_argument("--input_file", type=str, required=True, help="Path to local JSONL file")
    
    parser.add_argument("--output_file", type=str, default="processed_rlhf_dataset.jsonl")
...
def main():
    ...
    dataset = load_dataset("json", data_files={"train": args.input_file}, split="train")
    
    # process dataset
    print('initial length:', len(dataset))
    for row in dataset:
        print("DEBUG type(row):", type(row))
        print("DEBUG row:", row)
        break  # just inspect the first item
